...
async function main() {
  ...
  const norms = drawingInfo.normals;
  ...
  const nbuf = device.createBuffer({ size: norms.byteLength, usage: GPUBufferUsage.VERTEX | GPUBufferUsage.COPY_DST });
  device.queue.writeBuffer(nbuf, 0, norms);

  const cbuf = device.createBuffer({ size: cols.byteLength, usage: GPUBufferUsage.VERTEX | GPUBufferUsage.COPY_DST });
  device.queue.writeBuffer(cbuf, 0, cols);

  const ibuf = device.createBuffer({ size: inds.byteLength, usage: GPUBufferUsage.INDEX | GPUBufferUsage.COPY_DST });
  ...
  U.set(new Float32Array([kdScale, ksScale, LeScale, LaScale]), 36);
